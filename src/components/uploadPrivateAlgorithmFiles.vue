  ...
  cancelText="取消" :ok-button-props="{ style:{display: 'none' } }" :cancel-button-props="{ style:{display: 'none' } }">
    <div style="display: flex; flex-direction: row; padding: 10px">
      <span>
        ...
        <div style="padding-top: 0;">
          <a-radio-group v-if="form.algorithmType === '无量纲化'" v-model:value="form.useLog" name="gradioGroup" style="width: 200px">
      ...
      <!-- 点击相应按钮时，显示私有算法模版参考 -->
      <span>
        <p style="font-size: 15px; font-weight: bold;">上传算法模版参考</p>
        <div> 
          <a-button 
    ...
    <el-scrollbar :height="600">
      ...
      <div v-if="templateName === '故障诊断'" >
        <div>
          ...
          <h3>(1)对于深度学习模型，在获得模型参数的存放路径之后，通过该路径加载模型参数以初始化模型，以pytorch框架为例，通过如下方式加载模型参数</h3>
          <a-image :width="500" src="src/assets/fault-diagnosis-load-model.png" />
          <h3>(2)对于机器学习模型，在获得模型的存放路径之后，通过joblib库的'load()'方法加载已训练好的模型</h3>
          ...
          <h2>3. 模型推理及输出结果</h2>
          <h3>初始化模型之后，即可使用样本数据进行模型推理，进行模型推理之后，1代表有故障，0代表无故障，</h3>
          <h3>在此应该注意，注意深度学习和机器学习的故障诊断输入样本数据不同：</h3>
          <h3>(1)对于深度学习，其输入样本为原始的振动信号，因此使用numpy库的'numpy.load()'函数读取输入数据</h3>
          <h3>(2)对于机器学习，其输入样本为提取的手工特征，主程序中保存为pickle对象，因此使用pickle库的'pickle.load()'函数读取输入数据</h3>
          <h3>以使用pytorch框架训练的深度学习故障诊断模型为例，给出故障诊断示例代码如下</h3>
          <a-image :width="500" src="src/assets/fault-diagnosis-prediction.png"></a-image>
          <h3>在得到诊断结果之后，需要以打印的形式作为输出结果传回主程序</h3>
          <br>
          <h3>除此之外，对于机器学习的故障诊断，需要选定模型训练时的手工特征从输入数据中提取模型推理所需的特征样本，下图为机器学习的故障诊断推理示例</h3>
          <a-image :width="500" src="src/assets/fault-diagnosis-prediction-ml.png"></a-image>
      ...
      <div v-if="templateName === '故障预测'" >
        <div>
          ...
          <h2>2. 模型加载</h2>
          <h3>机器学习和深度学习的模型加载方式不同</h3>
          <h3>(1)如果是深度学习，在获得模型参数的存放路径之后，通过该路径加载模型参数以初始化模型，以pytorch框架为例，通过如下方式加载模型参数</h3>
          <a-image :width="500" src="src/assets/fault-diagnosis-load-model.png" />
          <h3>(2)如果是机器学习，在获得模型的存放路径之后，通过joblib库的'load()'方法加载已训练好的模型</h3>
          <a-image :width="500" src="src/assets/fault-diagnosis-load-model-2.png" />
          <h3>其中注意，如果需要使用gpu辅助计算，则需要指定gpu为gpu:0</h3>
          <h3></h3>
          <p style="font-size: 15px; font-weight: 600;">
          ...
          <h2>3. 模型推理及输出结果</h2>
          <h3>初始化模型之后，即可使用样本数据进行模型推理。这里注意深度学习和机器学习的样本数据不同</h3>
          <h3>(1)对于深度学习，其输入样本为原始的振动信号，因此使用numpy库的'numpy.load()'函数读取输入数据</h3>
          <h3>(2)对于机器学习，其输入样本为提取的手工特征，因此使用pickle库的'pickle.load()'函数读取输入数据</h3>
          <a-image :width="500" src="src/assets/fault-diagnosis-prediction.png"></a-image>
          <h3>其中，进行模型推理之后，得到预测的可能出现故障的时间，需要使用print()打印推理结果，以作为输出结果传回主程序，</h3>
          <h3>此时，需要注意的是在打印预测结果时，应该按照如下所示在预测结果之后紧跟'#'符号，以便于主程序读取输出的结果</h3>
      ...
      <div v-if="templateName === '无量纲化'" >
        <div>
          ...
          <h3>其中，如果需要使用到保存的训练数据的参数对样本数据进行无量纲化处理，则应通过读取'model-filepath'路径指向的pickle文件获取训练数据参数的记录</h3>
          <!-- <h3>在获取到原数据的存放路径后，通过numpy中的load()读取原数据，即可使用相应的插值算法对其进行插值处理。</h3> -->
          <h3>此外，在加载数据的时候，也需要根据处理的数据的类型分类讨论：</h3>
          <a-image :width="500" src="src/assets/normalization-input.png"></a-image>
          <h3>如上代码示例中，如果处理的数据为提取的信号特征样本，则应该将输入样本作为.pkl类型的文件进行读取，读取到的数据类型为pandas中的DataFrame；</h3>
          <h3>而如果处理的数据为信号序列，则应该将输入样本作为.npy类型的文件进行读取，读取到的数据类型为numpy数组</h3>
          <h2>2. 无量纲处理以及输出结果</h2>
          <h3>在进行无量纲处理时，根据处理的数据的类型不同，处理方式也不同：</h3>
          <a-image :width="550" src="src/assets/normalization-process-output.png" />
          <h3>(1)如果是对于提取的特征样本，则应该在步骤1中读取保存了训练数据的记录之后，再根据该记录的训练数据的参数对输入的样本进行无量纲化，以scikit-learn中的无量纲化方法为例，
          通过已加载的参数记录对象的'transform'方法对特征样本进行标准化。同时，应该注意要提前选定训练数据使用到的特征，如上所示代码中的'choose_features'。</h3>
          <h3>(2)如果是对于信号序列，以scikit-learn中的''无量纲化方法为例，将输入数据转换为合适的形状后即可对其进行无量纲化处理。需要注意的是，主程序输入的信号序列为(1, 2048)的numpy数组</h3>
          <h2>3. 专有算法代码模板示例</h2>
      ...
      <div v-if="templateName === '健康评估'" >
        <div>
          ...
          <h3>其余参数为层级评估模型所使用的各个参数，目前不建议修改，推荐使用默认参数。</h3>
          <h3>数据输入方法：通过python中的argparse库进行命令行参数的添加和解析</h3>
          <a-image :width="500" src="src/assets/health-evaluation-params-2.png" />
          
          <h2>2. 模型加载及模型推理</h2>
          
          <h3>层级评估模型的参数应该保存为.pkl的形式，并以.pkl文件的形式加载评估模型参数</h3>
          <a-image :width="500" src="src/assets/health-evaluation-load-model.png" />
        
          <h3>通过加载的评估模型，计算出权重矩阵，之后计算出层级权重指标，将计算得出的层级权重指标保存为.npy文件。</h3>
          <a-image :width="500" src="src/assets/health-evaluation-prediction.png" />
          <h2>3. 输出结果</h2>
          <h3>通过上述的模型推理得到层级权重指标矩阵之后，需要根据状态隶属度绘制出健康评估的可视化结果并给出相关建议。这里注意需要绘制层级指标权重柱状图，</h3>
          ...
          <a href="src/assets/exampleCode/My-HE-Algorithm-1.py" download="example-health-evaluation.py">点击下载示例代码源文件</a>
      ...
      <div v-if="templateName === '小波变换'" >
        <div>
          ...
          <h2>2. 数据处理及结果返回</h2>
          <h3>在通过'input-filepath'读取输入数据后，就可以根据定义的数据处理方法处理输入数据。
          然后将处理结果保存为.npy的文件以返回给主程序</h3>
          ...
          <a href="src/assets/exampleCode/My-Wavelet-1.py" download="example-wavelet-transform.py">点击下载示例代码源文件</a>
...
const beforeUpload = (file: any) => {
  ...
  if (isFaultDiagnosis || isFaultPrediction || isHealthEvaluation) {
    ...
    if (fileNum == 0 && !isPyFile && !isPklFile && !isPthFile ){
      message.error('请上传包含该' + algorithmType + '算法(.py类型)或是模型参数(.pth或是.pkl类型)的文件');
      return false;
    }
    //判断是否是用户上传私有算法时是否同时上传了故障诊断的模型以及相关的模型参数文件（共两个文件）
    if (fileNum == 1) {
      if (!(isPyFile && (ruleOfFDA == -1)) && !((isPthFile || isPklFile) && (ruleOfFDA == 1)) && !(isPyFile && (ruleOfHev == -1)) && !(isPklFile && (ruleOfHev == 1))){
        message.error('上传' + algorithmType + '算法时，需要上传定义的' + algorithmType + '算法(.py文件)以及诊断时使用的模型的加载参数(.pkl或.pth文件)');
        return false;
      }
    }

    if (fileNum == 2){
      message.error('上传' + algorithmType + '算法时，最多只能上传包含该算法的源文件和模型参数文件');
      return false;
    }
    ...
    if (!useLog){
      ...
      if (fileNum == 0 && !isPyFile && !isPklFile){
        message.error('请上传包含该' + algorithmType + '算法(.py类型)或是模型参数(pkl类型)的文件');
        return false;
      }
      //判断是否是用户上传保存了训练数据参数的无量纲化算法时是否同时上传了无量纲化算法以及相关的模型文件（共两个文件）
      if (fileNum == 1) {
        if (!(isPyFile && (ruleOfNor == -1)) && !( isPklFile && (ruleOfNor == 1))){
          message.error('上传' + algorithmType + '算法时，需要上传定义的' + algorithmType + '算法(.py文件)以及诊断时使用的模型的加载参数(.pkl文件)');
          return false;
        }
      }
      if (fileNum == 2){
...
const handleUpload = () => {
  ...
  if( fileNum == 1 ){
    ...
    if(algorithmType != '故障诊断' && algorithmType != '故障预测' && !(algorithmType=='无量纲化' && form.value.useLog) && algorithmType != '健康评估'){
      //上传除故障诊断、故障预测、健康评估、保存训练数据参数的无量纲化以外的算法时，只能上传一个包含该算法的python源文件
      if (fileList.value){
      ...
      if (fileList.value){
        ...
        if(isPyFile){
          if(algorithmType=='无量纲化' && form.value.useLog){
            message.error('如果上传保存了训练数据参数的无量纲化方法，则需要同时上传模型文件(.pkl类型)')
            return
          }
          message.error('上传' + algorithmType + '算法时，需上传模型参数文件(.pkl或.pth类型)');
          return;
        }
        if(isPklFile || isPthFile){
          message.error('上传' + algorithmType + '算法时，需上传定义的' + algorithmType + '算法(.py类型)')
          return;
        }
      }
    }
    ...
    if(fileList.value && fileNum){
      ...
      if(algorithm !== modelParams){
        ...
        message.error('上传定义的' + algorithmType + '算法(.py文件)以及使用的模型的加载参数(.pkl或.pth文件)时，两个文件名称需要保持一致');
        return;
      }
    }
  }
