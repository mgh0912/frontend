...
export const contrastOfAlgorithm = {
    ...
    '层次分析健康评估':
        "| 健康评估方法     | 优点                                                                                                       | 缺点                                                                 |\n"+
        "|------------|----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------|\n"+
        "| 层次分析模糊综合评估 | 能够处理不确定性和模糊性较强的信息，适用于那些难以用精确数值描述的情况。通过构建层次结构模型，可以系统地考虑多个影响健康的因素。结合了专家的知识（主观判断）和实际数据（客观信息），使得评价结果更加符合实际情况 | 需要构建复杂的层次结构，对于大规模的问题来说，计算量可能会非常大                                   |\n"+
        "| 层朴素贝叶斯评估   | 基于概率理论，假设特征之间相互独立，简化了模型。即使面对大量数据也能保持较快的处理速度。                                                             | 模型假设特征相互独立，但实际上很多情况下特征之间是有关联的，因此可能导致模型性能下降                         |\n"+
        "| 层次逻辑回归评估   | 模型的结果易于理解，可以通过系数大小直观地看出不同变量对结果的影响程度。                                                                     | 逻辑回归假设因变量与自变量之间存在线性关系，对于非线性关系可能不适合。由于逻辑回归采用最大似然估计法求解参数，因此对异常值较为敏感。 |",
    '小波变换':
        "| 小波变换的小波基类型         | 优点                                                     | 缺点                                              |\n"+
        "|--------------------|--------------------------------------------------------|-------------------------------------------------|\n"+
        "| Daubechies (db) 小波 | 较好的平衡了时间分辨率和频率分辨率，提供了从 db1 到 db20 的一系列小波，其中数字表示消失矩的数量。 | 随着消失矩数量增加，小波形状变得更加复杂，计算量也会相应增大。                 |\n"+
        "| Symlet (sym) 小波    | 相比于 Daubechies 小波，具有更好的对称性，有助于减少重构过程中出现的相位失真。          | 计算复杂度略高于部分 Daubechies 小波，需要根据具体应用选择适当的 symN 类型。 |\n"+
        "| Coiflet (coif) 小波  | 结合了 Daubechies 和 Symlet 的特点，同时优化了正交性和对称性。              | 计算相对复杂，支持的种类较少。                                 |\n"
...
// 校验算法组件完整性的流程
export const contenJsonForWaveletTrans = {
    schedule: ['小波变换']
